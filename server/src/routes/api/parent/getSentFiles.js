// src/routes/api/parent/getSentFiles.js
...
module.exports.getSentFiles = async (req, res) => {
  try {
    ...
    // Query the database for files sent by a parent to a doctor for a baby
    const result = await pool.query(
      `SELECT document_id, filename, mimetype, file_data, baby_id, uploaded_by, shared_with, is_from_doctor, created_at
         ...
         WHERE is_from_doctor = false AND shared_with = $1 AND baby_id = $2
