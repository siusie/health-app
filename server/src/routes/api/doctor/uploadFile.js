// src/routes/api/doctor/uploadFile.js
...
module.exports.uploadFile = async (req, res) => {
  try {
    const { doctorId, babyId, parentId } = req.params;
    ...
    const result = await pool.query(
      ...
      [file.originalname, file.buffer, file.mimetype, babyId, doctorId, parentId, true]
    );
    if (!result.rows[0]) {
      return res.status(404).json(createErrorResponse(404, 'File not found in database'));
    }
    return res.status(200).json(createSuccessResponse(result.rows[0]));
  } catch (error) {
